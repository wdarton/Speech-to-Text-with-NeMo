...
The script requires a CUDA-enabled NVIDIA GPU to run in real-time, as well as specific system packages for audio processing.
...
\# Create a virtual environment named 'asr\_env'  
python3 \-m venv asr\_env

\# Activate the virtual environment  
source asr\_env/bin/activate
...
pip install "nemo\_toolkit\[asr\]" pyaudio numpy scipy

## **Usage**

1. Save the provided Python script as realtime\_asr.py in your working directory.  
2. Ensure your microphone is connected and configured as the default input device on your Debian system.  
3. Run the script:

python realtime\_asr.py
...
* Once the speaker pauses (configurable via SILENCE\_PATIENCE in the script), the buffered audio is packaged and sent to **Parakeet-TDT-0.6b-v2**.  
